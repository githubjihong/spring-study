...
<body>
    ...
    document.getElementById("testBtn").addEventListener("click", ()=>{
    	...
    	if(formTypeSelect.value == '2') {
	    	const gdsNm = document.querySelector("input[name='gdsNm']");
	    	const tkLctn = document.querySelector("input[name='tkLctn']");
	    	const playerNm = document.querySelector("input[name='playerNm']");
	    	const hostOrg = document.querySelector("input[name='hostOrg']");
	    	const expectedAudience = document.querySelector("input[name='expectedAudience']");
	    	const expectedBudget = document.querySelector("input[name='expectedBudget']");
	    	const background = document.querySelector("textarea[name='background']");
	    	const requests = document.querySelector("textarea[name='requests']");
	    	const remarks = document.querySelector("textarea[name='remarks']");
	    	drftTtl.value="AESPA(에스파) 공연 기획안 심의 요청서";
	    	gdsNm.value="[AESPA] SYNK : HYPER LINE";
	    	tkLctn.value="서울 구로구 경인로 430 고척 스카이돔";
	    	playerNm.value="AESPA(에스파)";
	    	hostOrg.value="oHoTainer";
	    	expectedAudience.value="20000";
	    	expectedBudget.value="404000000";
	    	let backgroundHtml = ``;
	    	backgroundHtml += `1. 아티스트 그룹 \n`;
	    	backgroundHtml += `AESPA(에스파)\n`;
	    	backgroundHtml += `2.공연 유형 \n`;
	    	backgroundHtml += `콘서트 \n`;
	    	backgroundHtml += `3.공연명 \n`;
	    	backgroundHtml += `[AESPA] SYNK : HYPER LINE \n`;
	    	backgroundHtml += `4.공연가격 \n`;
	    	backgroundHtml += `VIP석 : 150,000원 / R석 : 100,000원 / S석 : 70,000원 \n`;
	    	backgroundHtml += `5.공연날짜 \n`;
	    	backgroundHtml += `2025년 5월 14일 14:00 / 19:00 \n`;
	    	backgroundHtml += `2025년 5월 15일 18:00 \n`;
	    	backgroundHtml += `6.공연시간 \n`;
	    	backgroundHtml += `200분`;
	    	background.value=backgroundHtml;
	    	
	    	let requestsHtml = ``;
	    	requestsHtml += `마케팅팀 : 콘서트 홍보 콘텐츠 기획 및 온/오프라인 광고 집행 (SNS/유튜브/포스터 등)\n`;
	    	requestsHtml += `디자인팀 : 포스터, 티켓, 공식 MD(굿즈) 디자인 제작 및 납기 조율\n`;
	    	requestsHtml += `팬클럽 운영팀 : 팬클럽 선예매 대상자 관리 및 공지사항 배포 협조\n`;
	    	requests.value=requestsHtml;
    	}
    	if(formTypeSelect.value == '3') {
