...
<body class="sidebar-mini" style="height: auto;">
					...
					<div class="modal-content">
						...
						<div class="modal-footer">
							...
							<button type="button" class="btn btn-primary"
								data-dismiss="modal" onclick="confirmEditPost()">저장</button>
						</div>
					</div>
				</div>
			</div>
	...
	function updateMember(memNo){
		fetch("/admin/member/memberEdit?memNo="+memNo).then(resp=>{
			...
			resp.json().then(data=>{
	            ...
	            let prevMemSecCodeNo = document.querySelector("#memSecCodeNoModal").value;
				...
				document.getElementById("testBtn").addEventListener("click", ()=>{
					...
					if(data.memFirstName == "중호") {
						document.getElementById("artActNm").value="송중호";
					}else if(data.memFirstName == "규방") {
						document.getElementById("artActNm").value="이규방";
					}else if(data.memFirstName == "선미") {
						document.getElementById("artActNm").value="선미";
						document.getElementById("artExpln").value="JYP엔터테인먼트 소속 4인조 걸그룹 원더걸스의 멤버로 포지션은 서브보컬이다.";
					}else {
						document.getElementById("artExpln").value="";
					}
	...
	function checkMemSecCodeNo(){
		...
		if(selectedValue === "M02"){
			...
			Swal.fire({
				icon: 'info',
				title: '회원이 아티스트로 변경되었습니다.',
				text: '아티스트 관련 입력 항목이 활성화됩니다.',
				confirmButtonText: '확인'
			});
