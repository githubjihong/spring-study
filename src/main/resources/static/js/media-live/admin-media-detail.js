...
function testBtn(type) {
	...
	if(type=='normal') {
		...
		// 게시글 제목 입력
		document.getElementById("post-title").value="[Show Champion] 선미 - 가시나 (SUNMI - GASHINA) l EP.244";
		//document.getElementById("post-title").value="[Show Champion] 선미 - 가시나 (SUNMI - GASHINA) l EP.244";
		
		let content = ``;
		content += `살아 숨쉬는 인간 장미꽃 선미!🌹\n`;
		content += `널 두고 어딜 가니ㅠ 진짜 너무 예쁜 가시나...!💖\n\n` ;
		content += `#쇼챔피언 #선미 #가시나\n\n`;
		content +=`★All about KPOP! Subscribe now★`;
		...
		document.getElementById("post-title").value = "250507 선미 oHoT 라이브 - SUNMI oHoT Live"
		document.getElementById("post-content").value = "#선미 #오핫라이브 #라방 #sunmi #oHoT ";
